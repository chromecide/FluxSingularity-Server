<?php
class KernelActionsAPIParseReadInput extends KernelObject{
	public function __construct($cfg=null){
		parent::__construct($cfg);
		
		$this->useDefinition('Object.Action');
		
		$this->setValue('ID','Kernel.Actions.API.ParseReadInput');
		$this->setValue('Name','Parse API.Read Inputs');
		$this->setValue('Description', 'Loads the values of an API Read request');
		$this->setValue('Author', 'Justin Pradier');
		$this->setValue('Version', '1.0.0');
		
		$this->addAttribute('Request', 'API.Read');
		$this->addAttribute('Session', 'Object.Session');
	...
	public function run(&$inputObject){
		...
		$readRequest = new KernelObject('API.Read');
